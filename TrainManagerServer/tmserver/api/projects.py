...
@router.get("/projects")
def api_get_projects(user: Annotated[TMUser, Depends(get_oauth2_user)]):
...
def api_create_project(req: CreateProjectRequest, user: Annotated[TMUser, Depends(get_oauth2_user)]):
    project = create_project(project_name=req.projectName, owner_id=user.id)
    return project

@router.get("/projects/{project_id}")
def api_get_project(project_id: int, user: Annotated[TMUser, Depends(get_oauth2_user)]):
...
def api_delete_project(project_id: int, user: Annotated[TMUser, Depends(get_oauth2_user)]):
    project = get_project(project_id)
    if project.owner_id != user.id:
        raise InvalidProjectError("Project id %i does not belong to user %s" % (project_id, user.username))
